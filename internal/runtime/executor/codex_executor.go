...
func (e *CodexExecutor) prepareCodexResponsesRequestBodyFast(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, from, to sdktranslator.Format, baseModel string, streamPatch codexStreamPatch, removeUnsupported bool, addImageTool bool) (originalPayload []byte, body []byte, ok bool, err error) {
	...
	// would hijack codexReasoningReplaySessionKey (which prefers a body prompt_cache_key),
	// breaking claude-session reasoning replay.
	finalizeCodexRequestObject(obj, baseModel, streamPatch, removeUnsupported, addImageTool, auth, "")
...
func (e *CodexExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	originalPayload, body, fastPath, prepErr := e.prepareCodexResponsesRequestBodyFast(ctx, auth, req, opts, from, to, baseModel, codexStreamTrue, true, addImageTool)
	if prepErr != nil {
		return resp, prepErr
	}
	...
	if !fastPath {
		...
		// prompt_cache_key is set later by cacheHelper (after applyCodexReasoningReplayCache),
		// so the reasoning-replay session key is not hijacked by a derived prompt_cache_key.
		body = finalizeCodexRequestBody(body, baseModel, codexStreamTrue, true, addImageTool, auth, "")
	...
	// The client input was already sanitized (fast path in prepareFast, slow path above).
	// applyCodexReasoningReplayCache only inserts trusted cached reasoning items into the
	// "input" array and never touches reasoning.effort, so a whole-body re-sanitize and a
	// second reporter scan here would be redundant. Report the reasoning effort once,
	// after replay, on the final body.
	body, replayScope := applyCodexReasoningReplayCache(ctx, from, req, opts, body)
	if fastPath {
		// The openai-response fast path produced valid JSON and reasoning replay is disabled
		// for it (claude-only), so the body is unchanged here and stays trusted-valid; skip
		// the untrusted whole-body gjson.ValidBytes scan when reporting the reasoning effort.
	...
	httpReq, upstreamBody, identityState, err := e.cacheHelper(ctx, from, url, auth, req, originalPayloadSource, body, cacheID)
	if err != nil {
		return resp, err
	}
...
func (e *CodexExecutor) executeCompact(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	body = helps.ApplyPayloadConfigWithRequest(e.cfg, baseModel, to.String(), from.String(), "", body, originalTranslated, requestedModel, requestPath, opts.Headers)
	cacheID := e.codexCacheID(ctx, from, req)
	// prompt_cache_key is set by cacheHelper below, keeping all codex paths consistent.
...
func (e *CodexExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	addImageTool := e.cfg == nil || e.cfg.DisableImageGeneration == config.DisableImageGenerationOff
	cacheID := e.codexCacheID(ctx, from, req)
	originalPayload, body, fastPath, prepErr := e.prepareCodexResponsesRequestBodyFast(ctx, auth, req, opts, from, to, baseModel, codexStreamKeep, true, addImageTool)
	if prepErr != nil {
		return nil, prepErr
	}
	originalPayloadSource := req.Payload
	...
	if !fastPath {
		...
		// prompt_cache_key is set later by cacheHelper (after applyCodexReasoningReplayCache),
		// so the reasoning-replay session key is not hijacked by a derived prompt_cache_key.
		body = finalizeCodexRequestBody(body, baseModel, codexStreamKeep, true, addImageTool, auth, "")
		// Fast path already sanitized obj["input"] before marshaling; only the slow
		// path needs the whole-body sanitize here.
		body = sanitizeOpenAIResponsesReasoningEncryptedContent(ctx, "codex executor", body)
	}
...
func (e *CodexExecutor) codexCacheID(ctx context.Context, from sdktranslator.Format, req cliproxyexecutor.Request) string {
	// Mirror cacheHelper's derivation exactly so the cacheID precomputed by the main
	// executor paths matches what cacheHelper would derive (image handlers / tests pass
	// "" and re-derive there).
...
func (e *CodexExecutor) cacheHelper(ctx context.Context, from sdktranslator.Format, url string, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, userPayload []byte, rawJSON []byte, cacheID string) (*http.Request, []byte, codexIdentityConfuseState, error) {
	var cache helps.CodexCache
	// cacheID is derived once per request via codexCacheID and threaded in by the main
	// executor paths; reuse it to avoid re-scanning req.Payload. Callers that do not
	// precompute it (image handlers, tests) pass "" and the same derivation runs here.
	if cacheID != "" {
		cache.ID = cacheID
	} else if sourceFormatEqual(from, sdktranslator.FormatClaude) {
		if cached, ok := codexClaudeCodePromptCache(req); ok {
			cache = cached
		}
	} else if sourceFormatEqual(from, sdktranslator.FormatOpenAIResponse) {
